        ...
        with col_main:
            ...
            # ---------------- TAB 2: Fingerprints ----------------
                ...
                if actions.empty:
                    ...
                    df_detail = (
                        df_filtered.loc[slow, ["timestamp", "query_id", "fingerprint", "query_type"]]
                    ...
                    df_detail["query_type"] = df_detail["query_type"].astype("string").str.upper()
                    df_detail["suggested_action"] = df_detail["query_type"].apply(
                                    lambda qt: (print(f"Missing key: {qt}") or "") if suggested_optimizations.get(qt) is None else ",".join(suggested_optimizations[qt])
                                )
